...
void EXT_FUNC SV_ConnectClient_internal(void)
{
	...
	s = Cmd_Argv(3);
	char protinfoErrorMsg[512];
	if (!Info_IsValid(s, protinfoErrorMsg, sizeof(protinfoErrorMsg)))
	{
		Con_Printf("[Connect Rejected] %s: protinfo validation failed: %s\n", NET_AdrToString(adr), protinfoErrorMsg);
		Con_Printf("[Connect Rejected] ProtInfo: %s\n", s);
	...
	s = Cmd_Argv(4);
	char userinfoErrorMsg[512];
#ifdef REHLDS_FIXES
	// truncate to 255 before sanity checks
	if (Q_strlen(s) > MAX_INFO_STRING - 1 || !Info_IsValid(s, userinfoErrorMsg, sizeof(userinfoErrorMsg)))
#else
	if (Q_strlen(s) > 256 || !Info_IsValid(s, userinfoErrorMsg, sizeof(userinfoErrorMsg)))
#endif
	{
		Con_Printf("[Connect Rejected] %s: userinfo validation failed: %s\n", NET_AdrToString(adr), userinfoErrorMsg);
		Con_Printf("[Connect Rejected] UserInfo: %s\n", s);
...
void EXT_FUNC SV_ActivateServer_internal(int runPhysics)
{
    ...
    PlayerReporter_ServerActivated();
