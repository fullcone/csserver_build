...
void CSteam3Server::Activate()
{
	...
	if (m_bLoggedOn)
	{
		...
		{
			m_bLanOnly = bLanOnly;
			m_bWantToBeSecure = COM_CheckParm("-secure") && !bLanOnly; // Default: disabled, use -secure to enable
		}
	}
	else
	{
		...
		m_bWantToBeSecure = COM_CheckParm("-secure") && !m_bLanOnly; // Default: disabled, use -secure to enable
